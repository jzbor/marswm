...
pub trait X11Window {
    ...
    fn x11_read_property_long(
        &self,
        display: *mut xlib::Display,
        property: X11Atom,
        prop_type: c_ulong,
    ) -> Result<Vec<u64>>;
    fn x11_read_property_string(
        &self,
        display: *mut xlib::Display,
        property: X11Atom,
    ) -> Result<String>;
    fn x11_replace_property_long(
        &self,
        display: *mut xlib::Display,
        property: X11Atom,
        prop_type: c_ulong,
    ...
    fn x11_geometry(
        &self,
        display: *mut xlib::Display,
    ) -> Result<(u64, i32, i32, u32, u32, u32, u32)>;
    ...
    fn x11_message(
        &self,
        display: *mut xlib::Display,
        msg_type: atoms::X11Atom,
        msg_format: c_int,
        msg_data: xlib::ClientMessageData,
    );
    ...
    fn x11_wm_normal_hints(
        &self,
        display: *mut xlib::Display,
    ) -> Result<(xlib::XSizeHints, c_long)>;
}
...
impl X11Window for xlib::Window {
    ...
    fn x11_get_state(&self, display: *mut xlib::Display) -> Result<u64> {
        let result =
            self.x11_read_property_long(display, WMState, WMState.to_xlib_atom(display))?;
        Ok(result[0])
    }
    ...
    fn x11_get_text_list_property(
        ...
        unsafe {
            ...
            } else if xlib::Xutf8TextPropertyToTextList(
                display,
                text.as_ptr(),
                &mut data_ptr,
                &mut nitems,
            ...
            {
                Err(MarsError::failed_conversion(
                    text.as_ptr(),
                    stringify!(*mut *mut i8),
                    stringify!(xlib::TextProperty),
                ))
    ...
    fn x11_net_wm_state(&self, display: *mut xlib::Display) -> Result<Vec<X11Atom>> {
        let atoms = self
            .x11_read_property_long(display, NetWMState, xlib::XA_ATOM)?
            .iter()
            .filter_map(|a| X11Atom::from_xlib_atom(display, *a))
            .collect();
        Ok(atoms)
    }
    ...
    fn x11_read_property_long(
        ...
        while first_run || remaining_bytes != 0 {
            unsafe {
                ...
                if status != 0 {
                    return Err(MarsError::failed_request(stringify!(
                        xlib::XGetWindowProperty
                    )));
                } else if actual_type == XLIB_NONE && actual_format == 0 && remaining_bytes == 0 {
                    return Err(MarsError::property_unavailable(property));
                } else if actual_type != prop_type || actual_format != 32 {
                    return Err(MarsError::invalid_response(stringify!(
                        xlib::XGetWindowProperty
                    )));
                } else {
                    data.extend_from_slice(slice::from_raw_parts(
                        data_ptr as *mut u64,
                        nitems.try_into().unwrap(),
                    ));
    ...
    fn x11_set_text_list_property(
        ...
        let mut pointers: Vec<*mut c_char> = list
            .iter()
            .flat_map(|s| CString::new(s.clone()))
            .map(|s| s.into_raw())
            .collect();
    ...
    fn x11_geometry(
        ...
        unsafe {
            ...
            if xlib::XGetGeometry(
                display, *self, &mut root, &mut x, &mut y, &mut w, &mut h, &mut bw, &mut depth,
            ) != 0
            {
    ...
    fn x11_get_window_types(&self, display: *mut xlib::Display) -> Vec<X11Atom> {
        ...
        unsafe {
            ...
            let result = xlib::XGetWindowProperty(
                display,
                *self,
                NetWMWindowType.to_xlib_atom(display),
                0,
                8,
                xlib::False,
                xlib::XA_ATOM,
            ...
            if result == xlib::Success.into() && !win_types_ptr.is_null() {
                let temp = slice::from_raw_parts(
                    win_types_ptr as *mut xlib::Atom,
                    nitems.try_into().unwrap(),
                );
                types = temp.to_vec();
            }
        ...
        types
            .iter()
            .filter_map(|xa| X11Atom::from_xlib_atom(display, *xa))
            .collect()
    }
    ...
    fn x11_unmap(&self, display: *mut xlib::Display) {
        unsafe {
            xlib::XUnmapWindow(display, *self);
        }
    }
    ...
    fn x11_wm_normal_hints(
        ...
        unsafe {
            ...
            if xlib::XGetWMNormalHints(display, *self, size_hints.as_mut_ptr(), &mut supplied_hints)
                != 0
            {
